...
- **Authentication tokens.** On Obsidian 1.11.4 and newer, PATs and OAuth Device Flow tokens are stored via `app.secretStorage`, which is backed by the OS keystore (macOS Keychain, Windows DPAPI, Linux secret service). They are not written to the plugin's `data.json`, so they don't travel with the vault via Obsidian Sync, a file syncing service, another git plugin, or a backup, and they aren't readable by other community plugins. On older Obsidian builds the API doesn't exist and the token falls back to `<vault>/.obsidian/plugins/easy-git/data.json`, unencrypted, as in previous releases. The plugin sends tokens only to `api.github.com` and `github.com/login/...` (or your configured Forgejo/Gitea instance) via Obsidian's `requestUrl`. Reports of unintended exfiltration, logging, or storage-in-clear-where-it-shouldn't-be are all in scope.
